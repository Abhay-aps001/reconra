...
export interface InspectedFile { file_id: string; filename: string; source_type: string; columns: string[]; row_count: number; sample_rows: Record<string, string>[]; candidate_role: string; suggestions: ImportSuggestion[]; }
...
export function parseInspection(value: unknown): ImportInspection {
  ...
  return {
    ...
    files: raw.files.map((file) => {
      ...
      return {
        file_id: text(item.file_id), filename: text(item.filename), source_type: text(item.source_type), columns: item.columns.map(text), row_count: item.row_count,
