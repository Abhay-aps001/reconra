...
export function ImportWorkflow() {
  ...
  return <section className="data-panel import-workflow">
    ...
    {inspection && <div className="inspection-results"><p className="status-chip">INSPECTED · {inspection.import_id}</p>{inspection.files.map((file) => <article key={file.file_id} className="mapping-panel"><h3>{file.filename}</h3><p>{file.source_type} · {file.row_count} rows · Backend classification: <strong>{file.candidate_role}</strong></p><div className="table-scroll" tabIndex={0} role="region" aria-label={`${file.filename} source preview`}><table><thead><tr>{file.columns.map((column) => <th key={column} scope="col">{column}</th>)}</tr></thead><tbody>{file.sample_rows.map((row, index) => <tr key={index}>{file.columns.map((column) => <td key={column}>{row[column]}</td>)}</tr>)}</tbody></table></div><h4>Confirm column mapping</h4><div className="mapping-grid">{file.columns.map((column) => { const suggestion = file.suggestions.find((item) => item.source_column === column); return <label key={column}><span>{column} {suggestion && <small>{Math.round(suggestion.confidence * 100)}% · {suggestion.reason}</small>}</span><select aria-label={`${file.filename} ${column} mapping`} value={mappings[file.file_id]?.[column] ?? ""} disabled={busy || stage === "validated"} onChange={(event) => setMappings((current) => { const next = { ...current[file.file_id] }; if (event.target.value) next[column] = event.target.value; else delete next[column]; return { ...current, [file.file_id]: next }; })}><option value="">Do not map</option>{(fields[file.candidate_role] ?? []).map((field) => <option key={field} value={field}>{field}</option>)}</select></label>; })}</div></article>)}</div>}
